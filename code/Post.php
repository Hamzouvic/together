...
if(!isset($_GET['id'])) header('Location: PublicationsPage.php');
$postId = $_GET['id'];
include_once 'Publications.php';
$publication = new Publication;
$post = $publication->getById($postId);
    ...
    <div class="w-full max-w-6xl rounded bg-white shadow-xl p-10 lg:p-20 mx-auto text-gray-800 relative md:text-left">
        <div class="md:flex items-center -mx-10">
            ...
            <div class="w-full md:w-1/2 px-10">
                <div class="mb-10">
                    <h1 class="font-bold uppercase text-2xl mb-5"><?= $post['id']?><br>Waterproof Jacket</h1>
                    <p class="text-sm"><?= $post['titre']?></p>
                </div>
                <div>
                    ...
                    <div class="inline-block align-bottom">
                        <button class="bg-yellow-300 opacity-75 hover:opacity-100 text-yellow-900 hover:text-gray-900 rounded-full px-10 py-2 font-semibold"> Donate now</button>
                    </div>
                </div>
            </div>
        </div>
